...
import { ApttusModule, ConfigurationService, ApiService } from "@congacommerce/core";
import { CommerceModule } from "../../../ecommerce.module";
import { HttpClientTestingModule, HttpTestingController } from "@angular/common/http/testing";
import { HttpClient } from "@angular/common/http";
import { of } from "rxjs";
import { Users, User1, UserLocale, User2} from './mock/index';
...
describe('UserService',() =>{
    ...
    xit('getLanguage when user.Language is set in the user object', () => {
        let user: any;
        spyOn(userService,'me').and.returnValue(of(User1));
    ...
    xit('getLanguage when user.Language is not set in the user object or "defaultlanguage" is available. we get from getbrowserlocale()', () => {
        let user: any;
        spyOn(userService,'me').and.returnValue(of(Users));
        spyOn(configurationService,'get').and.returnValue(null)
        spyOn(userService,'getBrowserLocale').and.returnValue('en-US')
        userService.getLanguage().pipe(take(1)).subscribe(c=>{
            user= c;  
        })
        expect(user).toEqual('en-US');
    ...
    xit('getLocale returns the local of the user when Locale is available in userobject',() =>{
        let user: any;
        spyOn(userService,'me').and.returnValue(of(User1));
    ...
    xit('getLocale returns the local of the user, when Locale is not available in userobject we get it from defaultLanguage',() =>{
        let user: any;
        spyOn(userService,'me').and.returnValue(of(Users));
        spyOn(configurationService,'get').and.returnValue(null)
        spyOn(userService,'getBrowserLocale').and.returnValue('en-US')
    ...
    it('setLocale with commit set as true/false',() =>{
        let user:any;
        spyOn(userService,'me').and.returnValue(of(User1));
